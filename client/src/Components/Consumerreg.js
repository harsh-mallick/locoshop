...
const Schoolreg = () => {
  ...
  const postData = async (e) => {
    e.preventDefault();
    const { name_consumer, house, lane, locality, state, pincode, phonenumber, email, password } = user
    ...
    const res = await fetch("/consumer-signup", {
      ...
      body: JSON.stringify({
        name_consumer, house, lane, locality, state, pincode, phonenumber, email, password
      })
    });
  ...
  return (
    <>
        <div className='section' id='consumerreg_section'>
          <div className='h1'>Customer Register</div>
          <div className="conn" id='contten'>
            <div className="form" id='form'>
              <form action="" method='POST'>
                <div className="label" id='label1'>&nbsp;&nbsp;&nbsp; Enter Customer Name:</div>
                <i className='fas fa-solid fa-user'></i>&nbsp;<input type="text" name="name_consumer" id="name_consumer" className="schoolname" required placeholder='Enter Customer Name.....' value={user.name_consumer} onChange={handleInput} /><br /><br />

                <div className="label" id='label2'> Enter Address:</div>
                <i className='fas fa-solid fa-location-arrow'></i>&nbsp;<input type="text" name="house" id="house" className="schooladdress" required placeholder='House No.....' value={user.house} onChange={handleInput} /><br /><br />
                ...
                <i className='fas fa-solid fa-envelope'></i>&nbsp;<input type="email" name="email" id="schoolemail" className="schooladdress" required placeholder='Enter Customer Email.....' value={user.email} onChange={handleInput} /><br /><br />

                <div className="label" id='label5'> Enter  Password:</div>
                <i className='fas fa-solid fa-key'></i>&nbsp;<input type="password" name="password" id="schoolpassword" className="schooladdress" required placeholder='Enter Customer Password.....' value={user.password} onChange={handleInput} /><br /><br />
